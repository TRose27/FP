...
aufgelistet. Die Volumina der Ventile 4 und 6 sind im offenen Zustand angegeben. Im geschlossenen Zustand besizen sie folgende Volumina:
\begin{align*}
  V_\text{4,geschlossen} &= \SI{0.005\pm0.001}{\litre} \\
  V_\text{6,geschlossen} &= \SI{0.022\pm0.002}{\litre}.
\end{align*}
Die Volumina des Ventils 11 sowie der Messgeräte werden vernachlässigt.
...
\end{align*}
Bei den Messungen mit der Drehschieberpumpe ist das Ventil 6, sowie das in Abbildung \ref{fig:ps2} zu sehende Ventil 4 verschlossen.
Die Addition der verbleibenden Volumina ergibt ein Gesamtvolumen von $\SI{11.1\pm0.8}{\litre}$.
Bei den Messungen mit der Turbopumpe sind beide in Abbildung \ref{fig:ps1} zu sehenden Ventile 4 geschlossen. Das Ventil 6, sowie das in Abbildung \ref{fig:ps2} zu sehende Ventil 4 ist offen, jedoch
befinden sich dieses Ventil und alle angeschlossenen Bauteile hinter dem Ansaugstutzen der Turbopumpe und gehören deswegen nicht zum Messvolumen.
So ergibt sich für die Turbopumpe ein Gesamtvolumen von $\SI{10.2\pm0.8}{\litre}$.
...
\begin{table}
  ...
  \begin{tabular}[t]{c c c}
   \toprule
    {Bauteil} & {Volumen (offen) / $\si{\litre}$} \\
     \midrule
     \csvreader[no head,
     late after line=\\,
...
\FloatBarrier
\label{sec:drehevak}
Zur Bestimmung des Saugvermögens aus der Evakuierungsmessung wird Gleichung \eqref{eqn:pkurve} umgestellt und logarithmiert, so dass sich folgende Gleichung ergibt:
\begin{equation}
...
Der Fehler des logarithmischen Ausdrucks wird nach \eqref{eqn:gaußerr} mit der folgenden Formel bestimmt:

\begin{equation}
  \Delta \ln \Bigl(\frac{p(t)-p_E}{p_0-p_E}\Bigr) = \sqrt{\Bigl(\frac{\Delta p(t)}{(p(t)-p_E)}\Bigr)^2 + \Bigl(\frac{\Delta p_0}{(p_0-p_E)}\Bigr)^2 + \Bigl(\frac{(p(t)-p_0)\Delta p_E}{(p_0 - p_E)(p(t)-p_E)}\Bigr)^2}
  \label{eqn:lnperr}
\end{equation}


Die so bestimmten Werte befinden sich ebenfalls in Tabelle \ref{tab:drehevak}.
Nun werden die logarithmierten Drücke gegen die gemittelten Zeiten aufgetragen und mit der Gleichung $y=ax+b$ in zwei Bereichen optimiert.
...
\begin{table}
  ...
  \begin{tabular}[t]{c c c c c c c c}
   \toprule
    p(t) / $\si{\milli\bar}$ & $\ln \Bigl(\frac{p(t)-p_E}{p_0-p_E}\Bigr)$ & $t_1$ / $\si{\second}$ & $t_2$ / $\si{\second}$ & $t_3$ / $\si{\second}$ & $t_4$ / $\si{\second}$ & $t_5$ / $\si{\second}$ &  $\overline{t}$ / $\si{\second}$ \\
     \midrule
     \csvreader[no head,
     late after line=\\,
     late after last line=\\\bottomrule]%
...
\end{align*}
Der Parameter $a$ stellt hierbei die Steigung $\frac{\mathrm{d}p}{\mathrm{d}t}$ dar,damit lässt sich das Saugvermögen durch Vergleich mit \eqref{eqn:leck}
...
\end{equation}
bestimmt.
Damit wurden folgende Saugvermögen bestimmt:
\begin{align*}
...
\begin{table}
  ...
  \begin{tabular}[t]{c c c c c}
   \toprule
    p(t) / $\si{\milli\bar}$ & $t_1$ / $\si{\second}$ & $t_2$ / $\si{\second}$ & $t_3$ / $\si{\second}$ & $\overline{t}$ / $\si{\second}$ \\
     \midrule
     \csvreader[no head,
     late after line=\\,
     late after last line=\\\bottomrule]%
...
\end{align*}
Daraus werden erneut mittels \eqref{eqn:saugvermögen} und \eqref{eqn:saugvermögenerr} die Saugvermögen bestimmt:
\begin{align*}
...
\begin{table}
  ...
  \begin{tabular}[t]{c c c c c c c c}
   \toprule
    p(t) / $\SI{e-5}{\milli\bar}$ & $\ln \Bigl(\frac{p(t)-p_E}{p_0-p_E}\Bigr)$ & $t_1$ / $\si{\second}$ & $t_2$ / $\si{\second}$ & $t_3$ / $\si{\second}$ & $t_4$ / $\si{\second}$ & $t_5$ / $\si{\second}$ &  $\overline{t}$ / $\si{\second}$ \\
     \midrule
     \csvreader[no head,
     late after line=\\,
     late after last line=\\\bottomrule]%
...
\begin{align*}
  S_1 &= \SI{9.1\pm1.2}{\litre\per\second} \\
  S_2 &= \SI{19.2\pm2.5}{\litre\per\second} \\
  S_3 &= \SI{20.5\pm2.6}{\litre\per\second} \\
  S_4 &= \SI{16.4\pm2.1}{\litre\per\second}.
\end{align*}
\begin{figure}
    \centering
...
\begin{table}
  ...
  \begin{tabular}[t]{c c c c c}
   \toprule
    p(t) / $\SI{e-5}{\milli\bar}$ & $t_1$ / $\si{\second}$ & $t_2$ / $\si{\second}$ & $t_3$ / $\si{\second}$ & $\overline{t}$ / $\si{\second}$ \\
     \midrule
     \csvreader[no head,
     late after line=\\,
     late after last line=\\\bottomrule]%
 ...
 \begin{table}
   ...
   \begin{tabular}[t]{c c c c c}
    \toprule
     p(t) / $\SI{e-4}{\milli\bar}$ & $t_1$ / $\si{\second}$ & $t_2$ / $\si{\second}$ & $t_3$ / $\si{\second}$ & $\overline{t}$ / $\si{\second}$ \\
      \midrule
      \csvreader[no head,
      late after line=\\,
      late after last line=\\\bottomrule]%
...
\label{sec:vgl}
Die verschiedenen bestimmten Saugvermögen sind in Abbildung \ref{fig:drehvgl} für die Drehschieberpumpe und in Abbildung \ref{fig:turbovgl} für die Turbopumpe dargestellt.
\begin{figure}
    \centering
