...
Das abgelesene Maximum und die damit folgende maximale Flussdichte am Ort der Probe lautet
\begin{equation*}
...
\begin{table}[H]
    ...
    \end{tabular}
\end{table}\FloatBarrier
Die gemessenen Winkel für die beiden Proben sind in der Tabelle \ref{tab:Messergebnisse} zu finden. Des Weiteren sind in der Tabelle die Drehwinkel für die 
beiden Proben nach Gleichung \eqref{eqn:theta} berechnet und durch die jeweilige Dicke der Probe geteilt, damit die Werte normiert sind. In der letzten Spalte sind die 
Differenzen der normierten Winkel zu sehen, um den Faraday-Effekt der Leitungselektronen zu erhalten.
\begin{table}[H]
 \centering
 \caption{Auflistung der gemessenen Winkel zu den jeweiligen Wellenlängen und die sich daraus ergebenen Polarisationswinkel.}
 \label{tab:Messergebnisse}
 \begin{tabular}{c c c c c c c c}
  \toprule
  $\lambda\,/\,\mathrm{\upmu m}$ & $\phi_{1,\mathrm{rein}}\,/\,\mathrm{rad}$ & $\phi_{2,\mathrm{rein}}\,/\,\mathrm{rad}$ & 
  $\theta_{\mathrm{rein}}\,/\,\frac{\mathrm{rad}}{\mathrm{m}}$ & $\phi_{1,\mathrm{dot}}\,/\,\mathrm{rad}$ & $\phi_{2,\mathrm{dot}}\,/\,\mathrm{rad}$ 
  & $\theta_{\mathrm{dot}}\,/\,\frac{\mathrm{rad}}{\mathrm{m}}$ & $\theta_{\mathrm{norm}}\,/\,\frac{\mathrm{rad}}{\mathrm{m}}$\\
  \midrule
  \csvreader[respect sharp, late after line = \\, late after last line=\\\bottomrule]{data/daten2.csv}{}
  {\num{\csvcoli} & \num{\csvcolii} & \num{\csvcoliii} & \num{\csvcoliv} & \num{\csvcolv} & \num{\csvcolvi} & \num{\csvcolvii} & \num{\csvcolviii}}
 \end{tabular}
\end{table}\FloatBarrier
Die Gleichung \eqref{eqn:frei} lässt sich nach dem normierten Drehwinkel umstellen:
\begin{equation}
...
\end{equation*}
verwendet. Da alle $\theta_{norm}$ eigentlich positiv sein sollte, werden für die lineare Regression nur Messwerte verwendet, welche positiv sind.
Die Messwerte und die lineare Regression sind in Abbildung \ref{fig:Darstellung} zu sehen.
\begin{figure}[H]
    \centering
...
\begin{align*}
    a &= (4,01\pm0,68)\cdot 10^{14}\,\frac{\mathrm{rad}}{\mathrm{m^3}}\;,\\
    b &= (-1876\pm402)\,\frac{\mathrm{rad}}{\mathrm{m}}\;.
\end{align*}
...
\begin{equation}
    m^* = \sqrt{\frac{e_0^3}{8\pi^2\epsilon_0c^3}\cdot\frac{1}{a}\cdot\frac{NB}{n}} = \SI{9,06\pm0,76e-33}{\kilo\gram}\;.
\end{equation}
