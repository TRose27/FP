...
Der Aufbau aus \ref{fig:8} entpsricht nun einer Einheitszelle. In Abbildung \ref{fig:9} sind nun die Spektren für drei, vier und sechs dieser Einheitszellen mit jeweils allen drei
Blendendurchmessern dargestellt. Die Entwicklung der Bandstrukturen aus den Aufspaltungen ist dabei deutlich zu erkennen. Aus \ref{fig:8} wurden nun die spektralen Abstände der bindenden
und antibindenden Zustände bestimmt. Aus Abbildung \ref{fig:9} wurden die spektralen Breiten der entsprechenden Bänder bei sechs Einheitszellen bestimmt. Diese Daten sind in Tabelle
\ref{tab:9} zu finden.
\begin{figure}
\centering
\includegraphics[scale=0.5]{build/9.pdf}
\caption{In a) sind die Spektren für drei, in b) für vier und in c) für sechs Einheitszellen dargestellt. Die Ausbildung der Bandstrukturen in Abhängigkeit der Anzahl an Einheitszellen wird deutlich.}
\label{fig:9}
\end{figure}
\FloatBarrier
\begin{table}
  \centering
  \caption{Spektrale Breiten der bindenden und antibindenden Zustände, sowie der entsprechenden Bänder}
  \label{tab:9}
   \noindent\makebox[\textwidth]{
\begin{tabular}[t]{c | c c c c c c}
 \toprule
 &\multicolumn{2}{c}{Band 1}  &\multicolumn{2}{c}{Band 2}  &\multicolumn{2}{c}{Band 3} \\
 \cmidrule(lr){2-3}  \cmidrule(lr){4-5}  \cmidrule(lr){6-7}
   {$d_\text{Blende}$ / $\si{\milli\metre}$} & {$\Delta f$ / $\si{\hertz}$} & {$\Delta f_\text{Band}$ / $\si{\hertz}$} & {$\Delta f$ / $\si{\hertz}$} & {$\Delta f_\text{Band}$ / $\si{\hertz}$} & {$\Delta f$ / $\si{\hertz}$} & {$\Delta f_\text{Band}$ / $\si{\hertz}$}\\
   \midrule
  10 & 491  & 859  & 315 & 524  & 268 & 434  \\
  13 & 747  & 1273 & 524 & 923  & 457 & 792  \\
  16 & 1004 & 1701 & 791 & 1358 & 703 & 1214 \\
    \bottomrule
 \end{tabular}
 }
\end{table}
\FloatBarrier
Diese Daten bestätigen zum Einen die Beobachtungen, welche bei Abbildung \ref{fig:8} gemacht wurden, des weiteren wird deutlich, dass die Energieaufspaltung bei Erhöhung der Anzahl an Einheitszellen
größer wird, da die Bänder deutlich breiter sind als der Abstand zwischen den bindenden und antibindenden Zuständen.

In Abbildung \ref{fig:10} ist ein Spektrum für zwölf $\SI{50}{\milli\metre}$ Röhren zwischen welchen jeweils abwechselnd $\SI{13}{\milli\metre}$ und $\SI{16}{\milli\metre}$ Blenden platziert wurden.
Wird dies mit dem Spektrum für den selben Aufbau nur mit $\SI{13}{\milli\metre}$ Blenden aus Abbildung \ref{fig:3} verglichen, so fällt auf, dass die Bandstruktur, bis auf eine Lücke in der
Mitte des Bandes, die selbe ist. So lässt sich Schlussfolgern, dass die kleineren Blenden für die "einhüllende" Bandstruktur und die größeren Blenden für die Aufspaltung der Bänder verantwortlich sind.
\begin{figure}
\centering
\includegraphics[scale=0.9]{build/10.pdf}
\caption{Spketrum für zwölf Röhren mit alternierenden Blendendurchmessern. Zusätzlich zur Bandstruktur ist eine Aufspaltung innerhalb der Bänder zu erkennen.}
\label{fig:10}
\end{figure}
\FloatBarrier

Bei Abbildung \ref{fig:11} ahndelt es sich um ein Spektrum für einen Aufbau bestehend aus fünf Einheitszellen, welche aus einem $\SI{50}{\milli\metre}$ Rohr, einer $\SI{16}{\milli\metre}$ Blende,
einem $\SI{75}{\milli\metre}$ Rohr und wieder einer $\SI{16}{\milli\metre}$ Blende bestehen.
Wird dieses SPektrum, mit den einzelnen Spektren für die einzelnen Röhren in den Abbildungen \ref{fig:6} und \ref{fig:7} verglichen, lassen sich die einzelnen Bänder den Resonanzen aus den einzelnen Röhren
zuordnen. So lassen sich die Bänder ab $\SI{2}{\kilo\hertz}$ folgendermaßen zuordnen: Das erste und sechste Band geht auf Resonanzen in der $\SI{50}{\milli\metre}$ Röhre zurück. Das zweite, dritte, fünfte und siebte
Band geht auf die Resonanzen in der $\SI{75}{\milli\metre}$ Röhre zurück. Das vierte Band geht auf beide Röhren zurück, da beide Röhren an dieser Stelle eine Resonanz vorweisen, weshalb es auch deutlich breiter als
die anderen Bänder ist.
\begin{figure}
\centering
\includegraphics[scale=0.9]{build/11.pdf}
\caption{Spektrum für alternierende Röhrenlängen bei gleich bleibenden Blendendurchmessern. Verschiedene Bänder gehen dabei auf Resonanzen in den verschiedenen Röhren zurück.}
\label{fig:11}
\end{figure}
\FloatBarrier

In Abbildung \ref{fig:12} sind verschiedene Defekte bei einem Aufbau von zwölf $\SI{50}{\milli\metre}$ Röhren dargestellt. Einmal ist dies eine $\SI{75}{\milli\metre}$ Röhre an
verschiedenen Positionen und einmal ist dies eine varrierende Rohrlänge an einer festen Position. Es ist zu beobachten, dass das verändern der Position des Defektes keine Auswirkungen
auf die Frequenz des Defektes im Spektrum hat. Eine Änderung der Größe des Defektes jedoch bewirkt eine Änderung der Frequenz des Defektes.
\begin{figure}
\centering
\includegraphics[scale=0.7]{build/12.pdf}
\caption{Spektren für verschiedene Defekte. In a) sind Spektren für einen Defekt selber Größe an verschiedenen Positionen dargestellt. Dieser ist als kleiner Peak vor $\SI{3}{\kilo\hertz}$ in der Bandlücke sichtbar. In b) sind Spektren für verschiedene Defektgrößen dargestellt.
Es ist im ersten Spektrum ein kleiner Peak am Ende der Bandlücke erkennbar, welcher auf den Defekt zurückgeht. Im zweiten Spektrum ist kein Defekt erkennbar, weshalb dieser wahrscheinlich innerhalb eines Bandes liegt. Im dritten Spektrum sind viele Irregularitäten zu erkennen,
höchstwahrscheinlich nicht auf den Defekt, sondern einen Messfehler zurückzuführen ist.}
\label{fig:12}
\end{figure}
\FloatBarrier
